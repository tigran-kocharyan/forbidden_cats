...
up = Updater("721020886:AAEFdaOC6LZcM9T2x-D5QpBl6MEvAwLZ8LU")
dp = up.dispatcher

def echo(bot, up):
    up.message.reply_text("Undefined...")
    bot.forwardMessage(chat_id=350279190,from_chat_id=up.message.chat_id,message_id=up.message.message_id)

def start(bot, up):
    up.message.reply_text("Hello! Do U wanna get some cats  (¬‿¬ )\nType /cat and U will get 'em...")

def helping(bot, up):
    up.message.reply_text('Only one command, dude, /cat...\nhmm, stop, actually two, nevermind...\nNOW THERE ARE SEVERAL  w(ﾟｏﾟ)w')
...
def ready(bot, up):
    ...
    #bot.sendMessage(chat_id=up.message.chat_id, text='ready', parse_mode=Markdown)

def test(bot, up):
    bot.send_message(chat_id=up.message.chat_id,text='*GOT IT!*', parse_mode=telegram.ParseMode.MARKDOWN)
    #bot.send_photo(chat_id=up.message.chat_id, photo=open('C:\\Users\\Tigran_K\\Desktop\\python 3\\bots\\third__\\image.jpg', 'rb'))
...
def sendgif(bot, up):
    bot.send_chat_action(chat_id=up.message.chat_id, action=telegram.ChatAction.UPLOAD_DOCUMENT)
    bot.sendAnimation(chat_id=up.message.chat_id, animation='https://lifeo.ru/wp-content/uploads/kotiki-87.gif')
